  ...
  useEffect(()=>{
    try {
      ...
      if(s){
        ...
        setLogs(p.logs||{}); setShopCk(p.shop||{});
        setReminders(p.reminders||[]); setVacDone(p.vacDone||{});
        setCheckDone(p.checkDone||{}); setMileDone(p.mileDone||{});
      }
    } catch{}
  },[]);

  useEffect(()=>{
    try { localStorage.setItem(SK, JSON.stringify({logs,shop:shopCk,reminders,vacDone,checkDone,mileDone})); } catch{}
  },[logs,shopCk,reminders,vacDone,checkDone,mileDone]);
